...
// iris-new: when embedded in the host app (iframe), intercept clicks on links
// that point at another Superset dashboard and emit a postMessage so the host
// can route in-app (tab/modal) instead of opening a new browser tab. No-op when
// not embedded. Wired via onClickCapture so it runs before React-Table handlers.
...
function tryEmitEmbeddedNavigation(e: MouseEvent<HTMLElement>): void {
  ...
  if (!m) return; // not a dashboard URL
...
export default function TableChart<D extends DataRecord = DataRecord>(
  ...
  return (
    <Styles onClickCapture={tryEmitEmbeddedNavigation}>
      <DataTable<D>
        columns={columns}
        data={data}
        rowCount={rowCount}
