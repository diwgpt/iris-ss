...
import { Filter, DataMask, DataMaskStateWithId } from '@superset-ui/core';
...
const ChartHeaderStyles = styled.div`
  ${({ theme }) => css`
    ...
    /* iris-new: inline per-chart time filter rendered in the header. Hide the
       filter's own label/title and tighten spacing so it reads as a compact
       control next to the chart title. */
    & > .embedded-chart-filter {
      flex: 0 0 200px;
      max-width: 240px;
...
const SliceHeader = forwardRef<HTMLDivElement, SliceHeaderProps>(
  (
    ...
    // iris-new: inline per-chart filter. Opt-in via dashboard metadata flag
    // `embed_chart_filters`. We render the single `filter_preset_or_custom`
    // native filter whose chartsInScope is exactly this chart, inline in the
    // header instead of in the dashboard-wide filter bar.
    const embedChartFilters = useSelector<RootState, boolean>(({ dashboardInfo }) =>
      Boolean((dashboardInfo as any)?.metadata?.embed_chart_filters),
    );
    const dataMaskSelected = useSelector<RootState, DataMaskStateWithId>(
      state => state.dataMask,
    );
    const embeddedFilter = useSelector<RootState, Filter | undefined>(state => {
      if (!embedChartFilters) return undefined;
      const cfg =
        (state.dashboardInfo as any)?.metadata?.native_filter_configuration ||
        [];
      return cfg.find(
        (f: any) =>
          f?.filterType === 'filter_preset_or_custom' &&
          Array.isArray(f.chartsInScope) &&
          f.chartsInScope.length === 1 &&
          f.chartsInScope[0] === slice?.slice_id,
      );
    });
    const handleEmbeddedFilterChange = (filter: Filter, dataMask: DataMask) => {
      // Apply immediately (bypass the FilterBar's Apply staging).
    ...
    return (
      <ChartHeaderStyles data-test="slice-header" ref={ref}>
        ...
        {embeddedFilter && (
          <div
            className="embedded-chart-filter"
          ...
          >
            <FilterControl
              ...
              filter={{
                ...embeddedFilter,
                dataMask: dataMaskSelected[embeddedFilter.id],
              }}
              onFilterSelectionChange={handleEmbeddedFilterChange}
