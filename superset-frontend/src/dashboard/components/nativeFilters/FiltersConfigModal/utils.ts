...
export const createHandleSave =
  ...
  async () => {
    const transformFilter = (id: string) => {
      ...
      // iris-new: preserve `chartsInScope` and `tabsInScope` across saves. These
      // are custom fields written into native_filter_configuration via SQL (the
      // inline-filter feature keys off chartsInScope). The form has no control
      // for them, so without this they get stripped on every modal Save — which
      // silently breaks every inline filter. Read from the original loaded config
      // (filterConfigMap) falling back to any value carried on the form inputs.
      const originalConfig = filterConfigMap[id] as unknown as {
        chartsInScope?: number[];
        tabsInScope?: string[];
      };
      const carryFromForm = formInputs as unknown as {
        chartsInScope?: number[];
        tabsInScope?: string[];
      };
      const chartsInScope =
        carryFromForm.chartsInScope ?? originalConfig?.chartsInScope;
      const tabsInScope =
        carryFromForm.tabsInScope ?? originalConfig?.tabsInScope;
      ...
      return {
        ...
        ...(chartsInScope !== undefined ? { chartsInScope } : {}),
        ...(tabsInScope !== undefined ? { tabsInScope } : {}),
      };
      return undefined;
    };
