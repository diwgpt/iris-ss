...
class ChartDataRestApi(ChartRestApi):
    ...
    def _send_chart_response(  # noqa: C901
        ...
        if result_format in ChartDataResultFormat.table_like():
            ...
            # Resolve chart name for the download filename. We deliberately
            # do NOT use query_context.slice_ here: that goes through
            # ChartDAO.find_by_id which applies the ChartFilter permission
            # filter, so guest/embedded users (Public role) see slice_=None
            # and the filename falls back to a timestamp. A direct PK lookup
            # works for everyone, since the user already has access to the
            # data being downloaded — exposing the chart name is harmless.
            chart_filename = None
            qc_form_data = getattr(result["query_context"], "form_data", None) or {}
            slice_id = qc_form_data.get("slice_id")
            if slice_id:
                slc = db.session.query(Slice).get(slice_id)
                if slc and slc.slice_name:
                    chart_filename = slc.slice_name
            ...
            if len(result["queries"]) == 1:
                ...
                if is_csv_format:
                    return CsvResponse(
                        data,
                        headers=generate_download_headers("csv", chart_filename),
                    )

                return XlsxResponse(
                    data,
                    headers=generate_download_headers("xlsx", chart_filename),
                )
            ...
            return Response(
                create_zip(files),
                headers=generate_download_headers("zip", chart_filename),
