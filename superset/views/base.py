...
def _sanitize_download_filename(name: str | None) -> str | None:
    # Strip header-breaking characters and trim. Keeps letters/digits/spaces
    # plus `-` and `_`; collapses runs of whitespace to single underscores.
    if not name:
        return None
...
def generate_download_headers(
    ...
    filename = safe if safe else datetime.now().strftime("%Y%m%d_%H%M%S")
